...
params: 
    ...
    ar.day25.deseq: '/group/testa/Project/CHD2/BulkRNAseq/results/PublicRepo/2.DEA/day25/Output/Savings/day25CbO.deseqARvsWT.rds'
    ar.day50.deseq: '/group/testa/Project/CHD2/BulkRNAseq/results/PublicRepo/2.DEA/day50/Output/Savings/day50CbO.deseqARvsWT.rds'
...
```{r, collapse=TRUE}
degs_ar_day25 <- data.frame(readRDS(params$ar.day25.deseq))[, c(2,6)]
degs_ar_day50 <- data.frame(readRDS(params$ar.day50.deseq))[, c(2,6)]

degs_ar_day25 <- tibble::rownames_to_column(degs_ar_day25, var = 'GeneSymbol')
degs_ar_day50 <- tibble::rownames_to_column(degs_ar_day50, var = 'GeneSymbol')

degs_ar_day25 <- degs_ar_day25 %>% filter(padj < padj_threshold)
degs_ar_day50 <- degs_ar_day50 %>% filter(padj < padj_threshold)

degs_ar_day25$dataset <- 'Day25_Ancestral_Organoids'
degs_ar_day50$dataset <- 'Day50_Ancestral_Organoids'
```

```{r}
degs <- purrr::reduce(list(degs_ar_day25, degs_ar_day50), full_join, by = "GeneSymbol")

degs <- degs %>%
  rename(log2FoldChange_day25CbO_Ar = log2FoldChange.x, padj.day25CbO_Ar = padj.x,
         log2FoldChange_day50CbO_Ar = log2FoldChange.y, padj.day50CbO_Ar = padj.y)
```
...
```{r}
columNamesVector <- c('Day25CbO_AR','Day50CbO_AR')
```
...
pdf(paste0(params$FiguresFolder, 'LysoAndAutophagyCoreGenes_heatmap.pdf'), width = 7, height = 15)
lysocoreheatmap
dev.off()
```

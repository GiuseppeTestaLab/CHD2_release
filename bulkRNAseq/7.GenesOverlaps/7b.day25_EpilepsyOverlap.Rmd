---
title: "CHD2 Cortical Brain Organoids Day25 - DEGs overlap with Epilepsy genes"
...
### Overlap Analysis between Epilepsy genes and DEGs in Day25 Cortical Brain Organoids
...
Dot <- ggplot(ResSel, aes(Module, KBase, label=Numbers)) + 
  ...
  scale_color_viridis_c(limits=c(1, 2), begin=0.4, option = 'A') + xlab('') + ylab('') +
  theme_bw() +
...
ggsave(Dot, filename=paste0(params$FiguresFolder, 'KB_Epilepsy_day25.pdf'), width=6.5, height=6.5)
```
...
```{r collapse=TRUE}
saveRDS(Res, paste0(params$SavingFolder, 'day25_EpilepsyOverlapResults.rds'))
```

```{r SaveSession, collapse=TRUE}
SessionInfo <- sessionInfo()
Date <- date()
save.image(paste0(params$SavingFolder, 'KB_Epilepsy_day25.RData'))
```

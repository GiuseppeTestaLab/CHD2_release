---
title: "Differential Expression Analysis of CHD2 iPSCs"
...
params: 
    ...
    SavingFolder: '/group/testa/Project/CHD2/BulkRNAseq/results/PublicRepo/2.DEA/iPSCs/Output/Savings'
    OutputFolder: '/group/testa/Project/CHD2/BulkRNAseq/results/PublicRepo/2.DEA/iPSCs/Output/Figures'
    padj_threshold: 0.05
...
```{r, collapse=TRUE}
SE_Bio_iPSC <- SE_Bio[, colData(SE_Bio)$Timepoint %in% 'd0']
SE_Bio_iPSC <- SE_Bio_iPSC[, !colData(SE_Bio_iPSC)$Genotype %in% 'HT']
colData(SE_Bio_iPSC)
```
...
```{r, collapse=TRUE}
CountMatrix <- assays(SE_Bio_iPSC)$counts
SampleMeta <- DataFrame(colData(SE_Bio_iPSC))

all(rownames(SampleMeta) == colnames(CountMatrix))
```

```{r, collapse=TRUE}
dds <- DESeqDataSetFromMatrix(countData=assays(SE_Bio_iPSC)$counts, DataFrame(colData(SE_Bio_iPSC)), design = ~Batch+Genotype)
mcols(dds) <- DataFrame(mcols(dds), DataFrame(rowData(SE_Bio_iPSC)))

dds$Genotype <- factor(dds$Genotype, levels = c("WT", "AR"))
...
```{r deseq_volcano_ar, fig.width=9, fig.height=9, message=FALSE, warning=FALSE}
...
dplyr::rename(as.data.frame(res_dds_ar), logFC = log2FoldChange, FDR = padj) %>% VolcanoTiltedNodash(. , AdjustedCutoff = padj_threshold, LabellingCutoff = 0.01, FCCutoff = log2fc_threshold, main = 'AR vs WT (iPSCs)') + labs(y='Log FoldChange') + xlim(0, -log10(FdrCeil)) + ylim(-logFcCeil, logFcCeil)
```

#### Ancestral LFC shrink

```{r deseq_volcano_ar ashr, fig.width=9, fig.height=9, message=FALSE, warning=FALSE}
#rename(as.data.frame(res_dds_ar), logFC = log2FoldChange, FDR = padj) %>% VolcanoTiltedNodash(. , AdjustedCutoff = 0.05, LabellingCutoff = 0.01, FCCutoff = 2, main = 'AR vs WT')

dplyr::rename(as.data.frame(resAshr_ar), logFC = log2FoldChange, FDR = padj) %>% VolcanoTiltedNodash(. , AdjustedCutoff = padj_threshold, LabellingCutoff = 0.01, FCCutoff = log2fc_threshold, main = 'AR vs WT (iPSCs)') + labs(y='Log FoldChange') + xlim(0, -log10(FdrCeil)) + ylim(-logFcCeil, logFcCeil)
```
...
# RDS
saveRDS(DEAList, paste0(SavingFolder, '/ipsc.', 'DEAList_AR.rds')) 

saveRDS(SE_deseq2, paste0(SavingFolder, '/ipsc.', 'SE_deseq2_AR.rds')) 

saveRDS(res_dds_ar, paste0(SavingFolder, '/ipsc.', 'deseqARvsWT.rds')) 

SessionInfo <- sessionInfo()
Date <- date()
save.image(paste0(SavingFolder, '/ipsc.', 'DEAnalysisWorkspace_AR.RData'))
```

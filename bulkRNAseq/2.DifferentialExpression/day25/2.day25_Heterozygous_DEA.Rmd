---
title: "Differential Expression Analysis of CHD2 day25 Cortical Brain Organoids"
...
params: 
    ...
    SavingFolder: '/group/testa/Project/CHD2/BulkRNAseq/results/PublicRepo/2.DEA/day25/Output/Savings'
    OutputFolder: '/group/testa/Project/CHD2/BulkRNAseq/results/PublicRepo/2.DEA/day25/Output/Figures'
    padj_threshold: 0.05
...
```
**`r print('Heterozygous')`**

 * Genes modulated considering a *FDR threshold of 0.1*: **`r dim(dplyr::filter(data.frame(res_dds_ht), padj < 0.1))[1]`**
 
 * Genes modulated considering a *FDR threshold of 0.05*: **`r dim(dplyr::filter(data.frame(res_dds_ht), padj < 0.05))[1]`**
 
 * Genes modulated considering a *FDR threshold of 0.05 and FC threshold of 1.5:* **`r dim(dplyr::filter(data.frame(res_dds_ht), padj < 0.05, abs(log2FoldChange)>log2(1.5)))[1]`**
 
 * Genes modulated considering a *FDR threshold of 0.05 and FC threshold of 2:* **`r dim(dplyr::filter(data.frame(res_dds_ht), padj < 0.05, abs(log2FoldChange)>log2(2)))[1]`**
 ...
 * Genes modulated considering a *FDR threshold of 0.1*: **`r dim(dplyr::filter(data.frame(resAshr_ht), padj < 0.1))[1]`**
 
 * Genes modulated considering a *FDR threshold of 0.05*: **`r dim(dplyr::filter(data.frame(resAshr_ht), padj < 0.05))[1]`**
 
 * Genes modulated considering a *FDR threshold of 0.05 and FC threshold of 1.5:* **`r dim(dplyr::filter(data.frame(resAshr_ht), padj < 0.05, abs(log2FoldChange)>log2(1.5)))[1]`**
 
 * Genes modulated considering a *FDR threshold of 0.05 and FC threshold of 2:* **`r dim(dplyr::filter(data.frame(resAshr_ht), padj < 0.05, abs(log2FoldChange)>log2(2)))[1]`**
...
dplyr::rename(as.data.frame(res_dds_ht), logFC = log2FoldChange, FDR = padj) %>% VolcanoTiltedNodash(. , AdjustedCutoff = padj_threshold, LabellingCutoff = 0.01, FCCutoff = log2fc_threshold, main = 'HT vs WT (day25 CO)') + labs(y='Log FoldChange') + xlim(0, -log10(FdrCeil)) + ylim(-logFcCeil, logFcCeil)
```
...
dplyr::rename(as.data.frame(resAshr_ht), logFC = log2FoldChange, FDR = padj) %>% VolcanoTiltedNodash(. , AdjustedCutoff = padj_threshold, LabellingCutoff = 0.01, FCCutoff = log2fc_threshold, main = 'HT vs WT (day25 CO)') + labs(y='Log FoldChange') + xlim(0, -log10(FdrCeil)) + ylim(-logFcCeil, logFcCeil)
```

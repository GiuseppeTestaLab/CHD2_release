...
```{r collapse=TRUE}
deseq.ht.ipsc <- readRDS(paste0(params$InputFolder, 'iPSCs/Output/Savings/ipsc.deseqHTvsWT.rds'))

deseq.ht.d25 <- readRDS(paste0(params$InputFolder, 'day25/Output/Savings/day25CbO.deseqHTvsWT.rds'))

deseq.ht.d50 <- readRDS(paste0(params$InputFolder, 'day50/Output/Savings/day50CbO.deseqHTvsWT.rds'))
```

### Make list of results dataframes
```{r collapse=TRUE}
deseqList <- list(deseq.ht.ipsc,
                  deseq.ht.d25,
                  deseq.ht.d50)

names(deseqList) <- c('deseq.ht.ipsc',
                  'deseq.ht.d25',
                  'deseq.ht.d50')
...
names(geneList) <- c('deseq.ht.ipsc',
                  'deseq.ht.d25',
                  'deseq.ht.d50')
```
...
print(results$iPSCs_HT@result$Description)
print(results$iPSCs_HT@result$p.adjust)
...
print(results$Day25_CbO_HT@result$Description)
print(results$Day25_CbO_HT@result$p.adjust)
...
print(results$Day50_CbO_HT@result$Description)
print(results$Day50_CbO_HT@result$p.adjust)
...
as.data.frame(results[1]) %>%
  DT::datatable(class='hover', rownames=FALSE, caption='GSEA results for iPSCs', 
...
as.data.frame(results[2]) %>%
  DT::datatable(class='hover', rownames=FALSE, caption='GSEA results for Day25 Cortical Organoids', 
                filter='top', escape=TRUE, extension='Buttons',
                options=list(pageLength=11, scrollX=T, dom='Bfrtip', buttons=list(I('colvis'))))

as.data.frame(results[3]) %>%
  DT::datatable(class='hover', rownames=FALSE, caption='GSEA results for Day50 Cortical Organoids', 
                filter='top', escape=TRUE, extension='Buttons',
                options=list(pageLength=11, scrollX=T, dom='Bfrtip', buttons=list(I('colvis'))))
```
...
```{r}
saveRDS(results, file = paste0(params$SavingFolder, 'GSEA_Heterozygous_results.Rds'))
```
